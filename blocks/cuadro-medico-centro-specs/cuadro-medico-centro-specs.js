 ...
 * Fila 0: [specialities (separated by |), provinceSlug]
 */
...
export default function decorate(block) {
  ...
  const provinceSlug = rows[0]?.children[1]?.textContent?.trim() || '';
  ...
  block.innerHTML = `
    ...
    <ul class="cm-cspecs-list">
      ${specs.map((s) => `<li class="cm-cspecs-item"><a href="/cuadro-medico/p/${provinceSlug}/pe/${toSlug(s)}">${s}</a></li>`).join('')}
    </ul>`;
}
